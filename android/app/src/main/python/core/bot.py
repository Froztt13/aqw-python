    ...
    async def handle_server_response(self, msg):
        ...
        if self.is_valid_json(msg):
            ...
            # on player spwaned in map
                ...
                if hasattr(self.player, "SKILLS") and isinstance(self.player.SKILLS, list):
                    while len(self.player.SKILLS) <= 5:
                        self.player.SKILLS.append({})
                    o_data = data.get("o", {})
                    self.player.SKILLS[5]["anim"] = o_data.get("anim", "")
                    self.player.SKILLS[5]["strl"] = o_data.get("strl", "")
                    self.player.SKILLS[5]["cd"] = o_data.get("cd", 0)
                    self.player.SKILLS[5]["tgt"] = o_data.get("tgt", "")
                    anim_strl = {
                        "anim": self.player.SKILLS[5].get("anim", ""),
                        "strl": self.player.SKILLS[5].get("strl", "")
                    }
