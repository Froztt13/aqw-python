...
class CoreTempleBot:
        ...
        while self.cmd.is_still_connected():
            ...
            while self.cmd.is_monster_alive() and check_master_in_cell and self.cmd.is_still_connected():
                ...
                if is_currently_taunter:
                    ...
                    soe_qty = getattr(item_enrage, "qty", 0) if item_enrage else 0
                    if soe_qty <= 0:
    ...
    async def prepare_items(self) -> bool:
        ...
        if self.is_taunter:
            item_soe = self.cmd.get_player().get_item_inventory("Scroll of Enrage")
            soe_qty = getattr(item_soe, "qty", 0) if item_soe else 0
            if soe_qty < 1:
...
class MidnightSunBot(CoreTempleBot):
    def __init__(self, cmd: Command, role: str, is_taunter: bool = False, target_monsters: str = None):
        super().__init__(cmd, role,
                         temple="midnightsun",
                         target_monsters=target_monsters or "Dying Light,Dawn Knight",
...
class SolsticeMoonBot(CoreTempleBot):
    def __init__(self, cmd: Command, role: str, is_taunter: bool = False, target_monsters: str = None):
        super().__init__(cmd, role,
                         temple="solsticemoon",
                         target_monsters=target_monsters or "Lunar Haze",
                         is_taunter=is_taunter)
