...
async def main(cmd: Command):
    map_name = "sanctuaryaiwass"    
    room_number = 9099
    item_to_farm: list[FarmTask] = [
        FarmTask(
            item_name="Sal Alembroth", 
            qty=1, 
            map_name=map_name,
            room_number=room_number,
            cell="r9", 
            pad="Top",
            monster_name="Mercury Prima",
            is_solo=True
        ),
        FarmTask(
            item_name="Milk of Sulfur", 
            qty=1, 
            map_name=map_name, 
            room_number=room_number,
            cell="r9", 
            pad="Top",
            monster_name="Sulfur Prima",
            is_solo=True
        ),
        FarmTask(
            item_name="Aeon Dream", 
            qty=1, 
            map_name=map_name, 
            room_number=room_number,
            cell="r11", 
    ...
    await cmd.register_quest(10688)
    # await cmd.register_quest(10689) # Member
